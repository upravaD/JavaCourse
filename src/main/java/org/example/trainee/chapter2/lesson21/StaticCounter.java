 ...
 *
 * <p>
 *     В языке программирования Java ключевое слово {@code static} означает,
 *     что конкретный член принадлежит самому типу, а не экземпляру этого типа.
 *     Это означает, что мы создадим только один экземпляр этого статического члена,
 *     который будет общим для всех экземпляров класса.
...
/**
 * <p>
 *     Plain Old Java Object - старый добрый Java-объект: простой объект класса,
 *     который не унаследован от какого-то специфического класса
 *     и не реализует никаких служебных интерфейсов сверх тех,
